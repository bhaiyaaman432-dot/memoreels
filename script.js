...
/* =========================================================
   FEATURED FILMS - AUTO FRAME EXTRACTOR
========================================================= */
...
filmItems.forEach((film) => {
  ...
  const posterSlot = film.querySelector('.film-poster-slot');
  ...
  // Important: Auto load data to grab frame
  preview.preload = 'auto';
  
  // Set crossOrigin so canvas doesn't get blocked
  ...
  /* --- MAGIC START: AUTO FRAME EXTRACTION --- */
  let frameExtracted = false;

  preview.addEventListener('loadeddata', () => {
    // Seek to 0.5 sec to grab a nice bright frame
  ...
  const captureFrame = () => {
    ...
    try {
      ...
      // Paint video frame on canvas
      ctx.drawImage(preview, 0, 0, canvas.width, canvas.height);

      // Set it as thumbnail image
      if (posterImg) {
        posterImg.src = canvas.toDataURL('image/jpeg', 0.85);
      }

      // Hide the black video player to show our fresh poster
      preview.style.opacity = '0';
      posterSlot?.classList.add('has-video');

    } catch (err) {
      console.warn('Frame catch failed, mostly due to browser restrictions:', err);
    }

    frameExtracted = true;
    preview.currentTime = 0; // Rewind
    preview.removeEventListener('seeked', captureFrame);
  };

  preview.addEventListener('seeked', captureFrame);
  /* --- MAGIC END --- */
  ...
  preview.addEventListener(
    'error',
    () => {
      posterSlot?.classList.add('video-error');
      preview.style.opacity = '0';
    },
    { once: true }
  );

  /* Desktop Hover Play Logic */
  const canHover =
  ...
  if (canHover) {
    film.addEventListener('pointerenter', () => {
      ...
      // Smoothly fade video IN on top of our poster
    ...
    film.addEventListener('pointerleave', () => {
      ...
      // Fade video OUT to show our poster again
      preview.style.opacity = '0';
      try {
